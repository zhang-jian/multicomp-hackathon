...
def load_audio_get_spectogram(audio_file, offset=0.0, duration=None):
    x, sr = librosa.load(audio_file, offset=offset, duration=duration, dtype=floatX)
    D = librosa.stft(x)
...
def get_spectogram_samples(audio_file, sample_len_secs, n_samples):
    ...
    sample_frames = [x[i:i+sample_len_frames] for i in [np.random.randint(0, len(x)-sample_len_frames) for _ in range(n_samples)]]
...
def load_dataset(root_path, sample_len_secs, n_samples_per_file):
    ...
    for i, genre in enumerate(GENRES):
        for audio_file in glob(os.path.join(root_path, genre, "*.au")):
            Xs.extend(get_spectogram_samples(audio_file, sample_len_secs, n_samples_per_file))
