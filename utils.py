...
def get_spectrogram_samples(audio_file, sample_len_secs, n_samples):
    ...
    sample_Ds = [librosa.stft(s, n_fft=N_FFT, hop_length=HOP_LENGTH) for s in sample_frames]
...
def convert_spectrogram_and_save(S, output_file):
    ...
    for i in trange(500, desc="Inverting spectrogram", ncols=80):
        Q = x * np.exp(1j*p)
        y = librosa.istft(Q, hop_length=HOP_LENGTH) + 1e-5
        p = np.angle(librosa.stft(y, n_fft=N_FFT, hop_length=HOP_LENGTH))
